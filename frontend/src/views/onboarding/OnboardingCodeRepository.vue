  ...
  <OnboardingLayout>
    <div class="mx-auto max-w-5xl">
      <div class="text-center">
        <h2 class="text-2xl font-bold text-slate-900">Connect Code Repositories</h2>
        <p class="mt-2 text-slate-600">
          Connect your code repositories to search code, issues, and pull requests
        </p>
      </div>
      ...
      <div v-else class="mt-12 grid gap-6 md:grid-cols-2">
        <IntegrationCard
          v-for="integration in codeRepositoryIntegrations"
...
const codeRepositoryIntegrations = ref([])

onMounted(async () => {
  try {
    await fetchIntegrations()
    codeRepositoryIntegrations.value = integrations.value.code_repository || []
  } catch (error) {
